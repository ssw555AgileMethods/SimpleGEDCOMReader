...
//	private int famChildrenWidth = 55;
	...
	//US33 list orphans
	public void printOrphans (ArrayList<Individual> indiList, ArrayList<Family> famList) throws Exception {
	  ArrayList<ArrayList<Individual>> result = new ArrayList<ArrayList<Individual>>();
	  for(int i=0; i<famList.size(); i++) { //get each family
      ArrayList<Individual> temp = new ArrayList<Individual>();
      Family fam = famList.get(i);
      if(fam.getChildrenId() == null) continue; //skip families without children
      String husb = fam.getHusbandId(); //get husband
      String wife = fam.getWifeId();
      Boolean husbAlive = LineProcessor.getIndiById(husb).getAlive();
      Boolean wifeAlive = LineProcessor.getIndiById(wife).getAlive();
	...
	//US34 print couples who were married when older was 2x age as younger
	public void printAgeGap (ArrayList<Individual> indiList, ArrayList<Family> famList) throws Exception {
	  ArrayList<ArrayList<Individual>> result = new ArrayList<ArrayList<Individual>>();
    for(int i=0; i<famList.size(); i++) { //get each family
      ArrayList<Individual> temp = new ArrayList<Individual>();
      Family fam = famList.get(i);
      if (fam.getMarried()==null) continue; //only get families who have a marriage
      String husb=fam.getHusbandId();
      int husbAge=LineProcessor.getIndiById(husb).getAge();
      String wife=fam.getWifeId();
      int wifeAge=LineProcessor.getIndiById(wife).getAge();
      if (husbAge>(2*wifeAge) || husbAge< (2*wifeAge)) {//more than twice as old or less than half as young
        temp.add(LineProcessor.getIndiById(husb));
        temp.add(LineProcessor.getIndiById(wife));
      }
      result.add(temp);
    }
    if(result == null) return;
